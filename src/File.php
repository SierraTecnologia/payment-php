...
class File extends ApiResource
{
    ...
    // versions, only `file` is used, but since sierratecnologia-php may be used with
    ...
    public static function create($params = null, $options = null): self
    {
        ...
        if (is_null($opts->apiBase)) {
            $opts->apiBase = SierraTecnologia::$apiUploadBase;
        }
        // Manually flatten params, otherwise curl's multipart encoder will
        // choke on nested arrays.
        // TODO: use array_column() once we drop support for PHP 5.4
        $flatParams = [];
        foreach (\SierraTecnologia\Util\Util::flattenParams($params) as $pair) {
