...
class CurlClient implements ClientInterface
{
    ...
    public function request($method, $absUrl, $headers, $params, $hasFile)
    {
        ...
        if (($method == 'post') && (SierraTecnologia::$maxNetworkRetries > 0)) {
        ...
        $opts[CURLOPT_CAINFO] = SierraTecnologia::getCABundlePath();
        if (!SierraTecnologia::getVerifySslCerts()) {
    ...
    {
        if ($numRetries >= SierraTecnologia::getMaxNetworkRetries()) {
            return false;
        }
    ...
    private function sleepTime(int $numRetries)
    {
        ...
        $sleepSeconds = min(
            SierraTecnologia::getInitialNetworkRetryDelay() * 1.0 * pow(2, $numRetries - 1),
            SierraTecnologia::getMaxNetworkRetryDelay()
        );
        ...
        $sleepSeconds = max(SierraTecnologia::getInitialNetworkRetryDelay(), $sleepSeconds);
    ...
    private function canSafelyUseHttp2()
    {
        ...
        // (cf. https://github.com/curl/curl/issues/2416), which SierraTecnologia use.
