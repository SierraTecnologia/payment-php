...
class SierraTecnologiaObject implements \ArrayAccess, \Countable, \JsonSerializable
{
    ...
    public function __set($k, $v)
    {
        ...
        $this->_values[$k] = Util\Util::convertToSierraTecnologiaObject($v, $this->_opts);
    ...
    public function &__get($k)
    {
        ...
        if (!empty($this->_values) && property_exists($this->_values, $k)) {
            ...
            $message = "SierraTecnologia Notice: Undefined property of $class instance: $k. "
                    ...
                    . "with the result returned by SierraTecnologia's API, "
                    . "probably as a result of a save(). The attributes currently "
                    . "available on this object are: $attrs";
            SierraTecnologia::getLogger()->error($message);
            return $nullval;
        } else {
            $class = get_class($this);
            SierraTecnologia::getLogger()->error("SierraTecnologia Notice: Undefined property of $class instance: $k");
            return $nullval;
        }
    }
    ...
    public function refreshFrom($values, $opts, $partial = false)
    {
        ...
        if ($values instanceof SierraTecnologiaObject) {
    ...
    public function updateAttributes($values, $opts = null, $dirty = true)
    {
        foreach ($values as $k => $v) {
            ...
            if (($k === "metadata") && (is_array($v))) {
                $this->_values[$k] = SierraTecnologiaObject::constructFrom($v, $opts);
            } else {
                $this->_values[$k] = Util\Util::convertToSierraTecnologiaObject($v, $opts);
            }
            if ($dirty) {
                $this->dirtyValue($this->_values[$k]);
            }
    ...
    public function serializeParamsValue($value, $original, bool $unsaved, $force, $key = null)
    {
        ...
        if ($value === null) {
            ...
            if (Util\Util::isList($value)) {
                ...
                return Util\Util::convertToSierraTecnologiaObject($value, $this->_opts)->serializeParameters();
            }
        } elseif ($value instanceof SierraTecnologiaObject) {
    ...
    {
        if ($recursive) {
            return Util\Util::convertSierraTecnologiaObjectToArray($this->_values);
        } else {
            return $this->_values;
        }
    }
    ...
    /**
     * Sets all keys within the SierraTecnologiaObject as unsaved so that they will be
