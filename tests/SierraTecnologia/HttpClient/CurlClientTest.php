...
use SierraTecnologia\HttpClient\CurlClient;
...
class CurlClientTest extends TestCase
{
    ...
    {
        $this->origMaxNetworkRetries = SierraTecnologia::getMaxNetworkRetries();
        $this->origMaxNetworkRetryDelay = SierraTecnologia::getMaxNetworkRetryDelay();
        $this->origInitialNetworkRetryDelay = SierraTecnologia::getInitialNetworkRetryDelay();
    }
    ...
    {
        $sierratecnologiaReflector = new \ReflectionClass('\SierraTecnologia\SierraTecnologia');

        $this->maxNetworkRetryDelayProperty = $sierratecnologiaReflector->getProperty('maxNetworkRetryDelay');
        $this->maxNetworkRetryDelayProperty->setAccessible(true);

        $this->initialNetworkRetryDelayProperty = $sierratecnologiaReflector->getProperty('initialNetworkRetryDelay');
        $this->initialNetworkRetryDelayProperty->setAccessible(true);

        $curlClientReflector = new \ReflectionClass('SierraTecnologia\HttpClient\CurlClient');
    ...
    {
        SierraTecnologia::setMaxNetworkRetries($this->origMaxNetworkRetries);
    ...
    {
        $fakeRandomGenerator = $this->getMock('SierraTecnologia\Util\RandomGenetator', ['randFloat']);
    ...
    public function testDefaultOptions()
    {
        ...
        $this->setExpectedException('SierraTecnologia\Error\Api', "Non-array value returned by defaultOptions CurlClient callback");
    ...
    {
        SierraTecnologia::setMaxNetworkRetries(2);
    ...
    public function testShouldNotRetryAtMaximumCount()
    {
        ...
        $this->assertFalse($this->shouldRetryMethod->invoke($curlClient, 0, 0, SierraTecnologia::getMaxNetworkRetries()));
    }
    ...
    public function testSleepTimeShouldGrowExponentially()
    {
        ...
        $this->assertEquals(
            SierraTecnologia::getInitialNetworkRetryDelay() * 1,
            $this->sleepTimeMethod->invoke($curlClient, 1)
        );
        $this->assertEquals(
            SierraTecnologia::getInitialNetworkRetryDelay() * 2,
            $this->sleepTimeMethod->invoke($curlClient, 2)
        );
        $this->assertEquals(
            SierraTecnologia::getInitialNetworkRetryDelay() * 4,
            $this->sleepTimeMethod->invoke($curlClient, 3)
        );
        $this->assertEquals(
            SierraTecnologia::getInitialNetworkRetryDelay() * 8,
    ...
    public function testSleepTimeShouldAddSomeRandomness()
    {
        ...
        $baseValue = SierraTecnologia::getInitialNetworkRetryDelay() * (0.5 * (1 + $randomValue));

        // the initial value cannot be smaller than the base,
        // so the randomness is ignored
        $this->assertEquals(SierraTecnologia::getInitialNetworkRetryDelay(), $this->sleepTimeMethod->invoke($curlClient, 1));
